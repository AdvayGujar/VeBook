...
const getAllBookingsByDate = async (req, res) => {
    try {
        const date = req.params.date; // Assuming date is sent as a query parameter

        if (!date) {
            return res.status(400).json({error: "Date parameter is required"});
        }

        const bookings = await Booking.findAll({where: {date: date}});

        if (bookings.length === 0) {
            return res.status(400).json({error: "No bookings found for the given date"});
        }
    ...
    try {
        ...
        const booking = await Booking.findOne({
            where: {
                ...
                status: {
                    [Op.or]: [1, 3] // Filters for status 1 or 3
                }
            },
        });
